...
for ip_address in "${ip_addresses[@]}"; do
    ...
    while [[ "$current_date" < "$stopped_before_date" ]]; do
        ...
        # Search for ping measurements in the current 2-day window
        ripe-atlas measurement-search --type ping --af 6 --target $ip_address --ids-only --limit 1000 --started-after $current_date --started-before $next_date >> all_measurement_ids.$ip_address
    ...
    # Calculate the average ping RTT
    if ((numbers == 0)); then
        avg=0
    else
    ...
    # Append the date and average ping RTT to output.txt
