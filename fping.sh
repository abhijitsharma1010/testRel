...
# Check if 10.1.1.1 is pinging
##if fping -C 2 -q 10.1.1.1 > /dev/null 2>&1; then
  ...
  # Ping the IP address
  fping -C 2 -q $IP > /dev/null 2>&1

  # Check the exit status of the fping command
  if [ $? -ne 0 ]; then
...
##else
  # If 10.1.1.1 is not pinging, exit the script
  ##exit 0
##fi
