...
if [ $ping_status -eq 0 ]; then
  ...
  if [ ! -f "$DOWNTIME_FILE" ]; then
    ...
    # Create the cache file and set its timestamp for timing the next notification
    echo "down" > "$CACHE_FILE"
    # Use date to explicitly set modification time to current time
    touch -d "@$current_time" "$CACHE_FILE"
    ...
    # Check when the last notification was sent using the timestamp of the cache file
    last_notification=0
    ...
    # Send update if it's been at least UPDATE_INTERVAL minutes since last notification
    if [ $minutes_since_last_notification -ge $UPDATE_INTERVAL ]; then
      ...
      echo "down" > "$CACHE_FILE"
      # Set the modification time to current time to properly track the next notification interval
      touch -d "@$current_time" "$CACHE_FILE"
    fi
  fi
else
