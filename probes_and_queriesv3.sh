...
while IFS= read -r PROBE_ID; do
    ...
    # Run the RIPE Atlas command and capture its output
    echo "Running RIPE Atlas command for Probe ID: $PROBE_ID" >> "$script_output_file"
    /root/.local/bin/ripe-atlas measure dns \
        --query-argument="$QUERY_ARG" \
        --query-type=A \
        --from-probes="$PROBE_ID" \
        --description="DNS query for ${QUERY_ARG}" \
        --timeout 10000 >> "$script_output_file" 2>&1

    # Check if the command was successful
    if [ $? -eq 0 ]; then
        echo "Command succeeded for Probe ID: $PROBE_ID" >> "$script_output_file"
    else
        echo "Command failed for Probe ID: $PROBE_ID" >> "$script_output_file"
    fi
