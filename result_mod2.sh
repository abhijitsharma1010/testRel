    ...
    if [[ $line == "Probe #"* ]]; then
        ...
        while IFS= read -r next_line; do
            ...
            if [[ $next_line == *"QUESTION SECTION:"* ]]; then
                # Read the next line to get the query
                IFS= read -r query_line
                # Extract the query (first field, remove the leading semicolon and trailing "IN A")
                query=$(echo "$query_line" | awk '{print $1}' | sed 's/;//g')
